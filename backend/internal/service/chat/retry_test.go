...
func TestRetryTurnDispatchesFailedPromptAsNewTurn(t *testing.T) {
	...
	settings := domain.ConversationSettings{
		Model: "current-model", ReasoningEffort: "high", ServiceTier: "default", ApprovalMode: domain.PermissionModeAcceptEdits,
	}
	...
	if sent[1].Settings.ServiceTier != settings.ServiceTier || sent[1].Settings.Model != settings.Model || sent[1].Settings.Effort != settings.ReasoningEffort || sent[1].Settings.Approval != settings.ApprovalMode {
